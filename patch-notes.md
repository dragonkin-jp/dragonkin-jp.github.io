...
### 未修正の既知バグ（2026年3月30日時点）
- **ルートフィルター停止問題**：起動後一定時間でフィルターが機能しなくなる不具合。2026年4月時点でも未修正・修正スケジュール未発表。コミュニティ回避策: 完全終了→再起動後にフィルター設定を再作成する
...
- **Bonus Summonsモディファイアのバグ**：グリッドにBonus Summonsモディファイアを付けると召喚物のAIが壊れて攻撃しなくなる不具合がコミュニティで報告されている（修正状況不明）。召喚ビルドを使う場合はこのモディファイアを避けること

### 次回パッチ
2026年3月30日時点で公式からの次回パッチ発表なし。
