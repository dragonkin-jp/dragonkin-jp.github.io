...
### RTX 40/50系 ブラックスクリーン

RTX 40/50系GPUでゲーム起動時にブラックスクリーンになる場合、以下を試してください。

1. Steam → ゲームプロパティ → 起動オプション に以下を追加して起動:
   ```
   -clearPSODriverCache -deleteuserpsocache
   ```
   起動後は削除推奨（一時的なキャッシュクリア）
2. グラフィックドライバを最新版に更新
3. 設定でFrame Generationを無効化（公式暫定対処）

*情報源: [Steam コミュニティ スレッド](https://steamcommunity.com/app/1863430/discussions/0/591764935569134494/)*
...
| 解像度 | 状況 |
|--------|------|
| 3440×1440（21:9） | 問題なく動作することを確認 |
| 5120×1440（32:9） | 3440×1440として動作するため左右に黒帯が発生 |

> ⚠️ 開発者からの公式声明・対応予定は現時点で未発表。
*情報源: [Steam コミュニティ - Ultrawide support?](https://steamcommunity.com/app/1863430/discussions/0/762934064830416119/)*
